...
public class Usuari extends JFrame {
	...
	public Usuari() {
		...
		biblio.llegir();
		LlistaSocis ls = biblio.getSocis();
		LlistaLlibres lli = biblio.getLlibres();
		LlistaReserves lr = biblio.getReserves();
		LlistaPrestecs lpa = biblio.getPrestecsActius();
		LlistaPrestecs lpi = biblio.getPrestecsInactius();
					...
					int seleccion = JOptionPane.showOptionDialog(
							...
							if (seleccion==0){
								...
								if(result==JOptionPane.YES_OPTION){
									...
									try {
										if (!lr.LlibreDisponible(reserva.getCodillibre(), reserva.getData())) {

											throw new LlibreNoDisponible(reserva.getCodillibre());
										}
										if (lr.NumRerservesDia(reserva.getData()) >= 30) {
											throw new ReservesDiaSuperior30(reserva.getData());

										}
										else {
											lr.AfegirReserva(reserva);
											JOptionPane.showMessageDialog(null, "Reserva realitzada amb exit", "Confirmacio", JOptionPane.INFORMATION_MESSAGE);
										}
										...
										JOptionPane.showMessageDialog(null, "El llibre no està disponible per aquesta data", "AVÍS", JOptionPane.ERROR_MESSAGE);
									} catch (ReservesDiaSuperior30 e) {
										JOptionPane.showMessageDialog(null, "Maxim de reserves per aquest dia", "AVÍS", JOptionPane.ERROR_MESSAGE);
									}
							...
							if (seleccion==1){
								...
								Prestec prestec = new Prestec(aux.getCodi(), nombre);
								int result = JOptionPane.showConfirmDialog(null, "Confirmar prestec el dia: " + data );
								
								if(result==JOptionPane.YES_OPTION){
									lpa.afegirPrestec(prestec);
									ls.retornaSoci(nombre).prestecUP();
		...
		public void mouseClicked(MouseEvent arg0) {
			...
			if(textField.getText().length()==0){
				for (int i = 0; i < lli.getNumllibres(); i++) {
					modelo.addElement(lli.return_libro(i));
				}
			}else{
				Llibre[] aux = lli.buscaLlibresPerNom(textField.getText());
				int i = 0;
						...
						if(result==JOptionPane.YES_OPTION){
							...
							try {
								...
								if (lr.NumRerservesDia(reserva.getData()) >= 30) {
									...
									lr.AfegirReserva(reserva);
									JOptionPane.showMessageDialog(null, "Reserva realitzada amb exit", "Confirmacio", JOptionPane.OK_OPTION);
								}
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(textField_1.getText().length()==0){
					for (int i = 0; i < lli.getNumllibres(); i++) {
						modelo2.addElement(lli.return_libro(i));
						
						
					}
				}else{
					Llibre[] aux = lli.PerTematica(textField_1.getText());
		...
		tabbedPane.addMouseListener(new MouseAdapter() {
			...
			if(tabbedPane.getSelectedIndex() == 2){
				Prestec[] n = lpa.prestecsUsuari(nombre);
				int i=0;
				if(n!=null){
					while(i<5 && n[i]!=null) {
						Llibre aux = lli.retornaLlibre(n[i].getId_llibre()).Duplicat();
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowActivated(WindowEvent arg0) {
				if (ls.existeix(nombre)){
					...
					String n = String.valueOf(ls.retornaSoci(nombre).getNum_prestecs());
		...
		addWindowListener(new java.awt.event.WindowAdapter() {
            ...
            public void windowClosing(java.awt.event.WindowEvent evt) {
            	...
            	if(result==JOptionPane.YES_OPTION){
            		try {
						lpa.guardarFitxer("prestecsactius.dat");
						lr.EscriureFitxerReserves();
						ls.guardar();
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
